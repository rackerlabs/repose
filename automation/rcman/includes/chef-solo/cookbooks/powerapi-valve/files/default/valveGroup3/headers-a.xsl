            ...
            <httpx:headers>
                <httpx:request>
                  <xsl:element name="httpx:header">
                    <xsl:attribute name="name"><xsl:value-of select="'translation-a'"/></xsl:attribute>
                    <xsl:attribute name="value"><xsl:value-of select="'a'"/></xsl:attribute>
                  </xsl:element>
                  <xsl:apply-templates select="httpx:request//httpx:header" />
                </httpx:request>
                <httpx:response>
                  <xsl:element name="httpx:header">
                    <xsl:attribute name="name"><xsl:value-of select="'translation-response-a'"/></xsl:attribute>
                    <xsl:attribute name="value"><xsl:value-of select="'response-a'"/></xsl:attribute>
                  </xsl:element>
                  <xsl:apply-templates select="httpx:request//httpx:header" />
                </httpx:response>
            </httpx:headers>
