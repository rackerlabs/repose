...
if [ -z "$1" ] ; then
  ...
  mkdir -p "$1/usr/share/lib/repose/filters"
  mkdir -p "$1/var/repose"
  mkdir -p "$1/var/log/repose"
  mkdir -p "$1/etc/repose"

  for f in "$1"/usr/share/lib/repose/* "$1"/usr/share/lib/repose/filters/*
  do
    if [ -f $f ]; then
      echo unlink $f
      unlink $f
  ...
  do
    echo ln -s $f "$1/usr/share/lib/repose/filters/"
    ln -s $f "$1/usr/share/lib/repose/filters/"
  done

  for f in $DIR/repose-aggregator/core/valve/target/repose-valve.jar
  do
    echo ln -s $f "$1/usr/share/lib/repose/"
    ln -s $f "$1/usr/share/lib/repose/"
  done
