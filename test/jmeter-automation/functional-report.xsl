...
<xsl:template match="testResults">
	<html>
		<head>
			<title>Test Results<xsl:if test="$file != ''"> - <xsl:value-of select="$file" /></xsl:if></title>
			<style type="text/css">
				body {
...
<xsl:template name="pageHeader">
	<h1>Test Results<xsl:if test="$file != ''"> - <xsl:value-of select="$file" /></xsl:if></h1>
	<table width="100%">
		<tr>
			<td align="left"></td>
	...
	<xsl:if test="$allFailureCount > 0">
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
			...
			<xsl:if test="$failureCount > 0">
                ...
                <pre><xsl:value-of select="responseData" /></pre>
				<table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
				<tr valign="top">
                    <th>Assertion</th>
				...
				<xsl:for-each select="/testResults/*[@lb = current()/@lb][attribute::s='false']">
                    <xsl:for-each select="./assertionResult[failure='true']">
                      <tr>
