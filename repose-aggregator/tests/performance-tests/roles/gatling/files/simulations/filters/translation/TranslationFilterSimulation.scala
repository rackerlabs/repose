...
/**
  * Translation filter performance simulation.
  */
class TranslationFilterSimulation extends AbstractReposeSimulation {
  ...
  val mainScenario = scenario("Translation Filter Test")
    .forever() {
      exec(getFooBarBaz)
    }
    .inject(
