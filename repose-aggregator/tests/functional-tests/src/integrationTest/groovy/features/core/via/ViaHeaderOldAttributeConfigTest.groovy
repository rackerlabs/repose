...
/**
 * Tests the contents of the Via header when it's configured using the old "via" attribute in the "deployment-config"
 * element in the container config.
 */
class ViaHeaderOldAttributeConfigTest extends ReposeValveTest {
    private static final String VIA_PREFIX = "Cactuar"
    ...
    def setupSpec() {
        ...
        def params = properties.getDefaultTemplateParams() + [viaPrefix: VIA_PREFIX]
        repose.configurationProvider.applyConfigs("common", params)
        repose.configurationProvider.applyConfigs("features/core/via", params)
        repose.configurationProvider.applyConfigs("features/core/via/oldattribute", params)
        repose.start()
