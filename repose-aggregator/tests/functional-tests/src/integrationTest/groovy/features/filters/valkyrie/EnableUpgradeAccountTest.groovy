...
class EnableUpgradeAccountTest extends ReposeValveTest {
    ...
    def setupSpec() {
        ...
        repose.configurationProvider.applyConfigs("features/filters/valkyrie/upgradeaccount", params);
    ...
    def "upgrade_account user request with an X-Device-Id header value not contained in the user's permissions will be permitted"() {
        ...
        fakeValkyrie.with {
            account_perm = "upgrade_account"
        }

        when: "a request is made against a device with Valkyrie set permissions"
        MessageChain mc = deproxy.makeRequest(url: reposeEndpoint + "/resource/99999", method: "DELETE",
                headers: [
    ...
    @Unroll
    def "upgrade_account user request with an X-Device-Id header value not contained in the user's permissions will not be permitted"() {
        given: "A device ID with a particular permission level defined in Valkyrie"
        fakeIdentityService.with {
            client_apikey = UUID.randomUUID().toString()
            client_token = UUID.randomUUID().toString()
        ...
        where:
        method << ["HEAD", "GET", "PUT", "POST", "PATCH"]
    }
