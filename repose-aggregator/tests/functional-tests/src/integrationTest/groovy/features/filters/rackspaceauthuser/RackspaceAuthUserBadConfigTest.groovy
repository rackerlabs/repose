...
class RackspaceAuthUserBadConfigTest extends ReposeValveTest {
    ...
    def "should fail to start with bad config"() {
        ...
        repose.configurationProvider.applyConfigs("features/filters/rackspaceauthuser/badconfig", params)
        repose.start()
        ...
        reposeLogSearch.searchByString("Invalid content was found starting with element").size() > 0
        reposeLogSearch.searchByString("RackspaceAuthUserFilter - RackspaceAuthUserFilter has not yet initialized.").size() > 0
    }
}
