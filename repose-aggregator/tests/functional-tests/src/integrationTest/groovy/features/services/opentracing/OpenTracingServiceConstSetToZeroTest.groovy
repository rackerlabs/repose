...
class OpenTracingServiceConstSetToZeroTest extends ReposeValveTest {
    ...
    def "when OpenTracing config is enabled with const sample=0, new span is not created"() {
        ...
        and: "request should not have tracer header because sampling const=0"
    ...
    @Unroll("Should return 200 with #method with a tracer id: #trace_id")
    def "when OpenTracing config is enabled with const sample=0, and trace id passed in, new span is not created, and trace id is passed through"() {
