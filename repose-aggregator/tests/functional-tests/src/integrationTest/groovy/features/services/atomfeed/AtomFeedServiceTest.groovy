...
class AtomFeedServiceTest extends ReposeValveTest {
    ...
    def "when the Atom feed response is chunked, all new entries should still be processed in order"() {
        ...
        atomEndpoint.defaultHandler = { Request request ->
            ...
            new Response(SC_OK, null, fakeAtomFeed.headers + ["Transfer-Encoding": "chunked"], chunkString(responseBody))
        }
    ...
    static String chunkString(String s, int chunkSize = 1024) {
        ...
        s = s.drop(chunkSize)
        while (chunkLength > 0) {
            sb.append(Integer.toHexString(chunkLength)).append("\r\n").append(chunk).append("\r\n")
            chunk = s.take(chunkSize)
