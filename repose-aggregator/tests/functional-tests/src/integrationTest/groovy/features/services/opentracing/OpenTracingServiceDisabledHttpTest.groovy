...
class OpenTracingServiceDisabledHttpTest extends ReposeValveTest {
    ...
    def setupSpec() {
        ...
        repose.configurationProvider.applyConfigs(
            "features/services/opentracing/disabled/http",
    ...
    def "when OpenTracing is disabled, new span is not created"() {
    ...
    @Unroll("Should return 200 with #method with an tracer id: #trace_id")
    def "when OpenTracing config is disabled, and trace id passed in, new span is not created, and trace id is passed through"() {
