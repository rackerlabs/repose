...
class CaseSensitiveHeadersTest extends ReposeValveTest {
    ...
    def setupSpec() {
        ...
        reposeConfigProvider.applyConfigs("features/filters/contentnormalization", params)
    ...
    @Unroll("Requests - headers: #headerName with \"#headerValue\" after normalization keep its case #headerNormalize")
    def "Requests - headers should keep its case in requests"() {
        ...
        mc.handlings[0].request.headers.getFirstValue(headerName) == headerNormalize


        where:
        headerName         | headerValue        |headerNormalize
        "Accept"           | "text/plain"       |"application/json"
        "ACCEPT"           | "application/JSON" |"application/JSON"
        "accept"           | "TEXT/plain;q=0.2" |"application/json"
