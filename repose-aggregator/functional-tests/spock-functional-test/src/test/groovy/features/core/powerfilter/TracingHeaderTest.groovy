...
class TracingHeaderTest extends ReposeValveTest {
    ...
    def "should pass a tracing header through the filter chain if one was provided"() {
        when:
        MessageChain mc = deproxy.makeRequest(url: reposeEndpoint, headers: ["x-request-guid": "test-guid"])

        then:
        mc.getHandlings().get(0).getRequest().getHeaders().getFirstValue("x-request-guid").equals("test-guid")
    }
    ...
    def "should pass a new tracing header through the filter chain if one was not provided"() {
        ...
        then:
        mc.getHandlings().get(0).getRequest().getHeaders().getFirstValue("x-request-guid").matches(".+-.+-.+-.+-.+")
    }
}
