...
class IntrafilterLoggingTest extends ReposeValveTest{
    ...
    def "Request entry with client-auth and ip-identity filter"() {
        ...
        assertHeadersExists(["Intrafilter-UUID"], authrespline1)
        assertKeyValueMatch([
                "currentFilter": "client-auth",
                "responseBody": "",
        ...
        assertHeadersExists(["Intrafilter-UUID"], authrespline2)
        assertKeyValueMatch([
                "currentFilter": "ip-identity",
                "responseBody": "",
                "httpResponseCode": "200"
    ...
    def "Request entry with client-auth and ip-identity filter with response body"() {
        given:
        def responseBody = "{\"response\": \"amazing\""
        when: "User passes a request through repose"
        MessageChain mc = deproxy.makeRequest(url: reposeEndpoint + "/servers/server123", method: 'GET',
                headers: ['X-Auth-Token': fakeIdentityService.client_token],
                defaultHandler: { request ->
                    return new Response(200, "OK", ["Content-Type": "application/json"], responseBody) })
