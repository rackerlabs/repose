...
[WARNING]
====
Python support is provided via Jython which contains a dependency on Apache commons-compress.
This dependency has a known vulnerability when reading zip files, and so it recommended not to read any zip files from the user.
