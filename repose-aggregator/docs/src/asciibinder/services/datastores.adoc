...
When you configure *Repose* to start with the Distributed Datastore, the running *Repose* instance will try to find the `<port>` configuration that matches it's own node ID in the <<../architecture/system-model.adoc#,System Model>>.
If the `node` attribute is not defined for a `<port>`, that `<port>` is considered the default `<port>`.
A running *Repose* instance will listen on the default `<port>` for Distributed Datastore traffic if no `<port>` is defined with a `node` attribute matching its own node ID in the <<../architecture/system-model.adoc#,System Model>>.
...
<10> Defines the default `port` on which all datastores will listen.
<11> Overrides the default `port`, defined for all datastores to listen, only for this particular `node`.
...
The distributed datastore will use a connection pool from the <<http-client.adoc#,HTTP Client Service>> to communicate across nodes.
...
Then all of the dynamic clients are configured with a remote datastore and the <<http-client.adoc#,HTTP Client Service>> configured to provide the proper authentication.
...
----
<?xml version="1.0" encoding="UTF-8"?>
<remote-datastore host="remote.example.com"
                  port="8080"/>
----
...
----
<?xml version="1.0" encoding="UTF-8"?>
<remote-datastore host="remote.example.com" <!--1-->
                  port="8080" <!--2-->
                  useHTTPS="true" <!--3-->
                  connection-pool-id="remote-datastore"/> <!--4-->
----
<1> Defines the host providing the remote datastore.
<2> Defines the port on which the remote datastore is listening (1-65535).
<3> Defines if the remote datastore is expecting a secure connection request. +
    Default: `true`
<4> Http Connection Pool ID to use when connecting to the Remote Datastore.

[NOTE]
====
Refer to the <<http-client.adoc#,HTTP Client Service>> and <<../recipes/client-authentication.adoc#, SSL/TLS Client Authentication>> documentation for properly securing the connection pool used to connect to the Remote Datastore.
====
