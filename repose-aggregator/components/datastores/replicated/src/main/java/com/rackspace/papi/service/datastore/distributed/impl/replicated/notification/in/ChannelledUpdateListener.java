package com.rackspace.papi.service.datastore.distributed.impl.replicated.notification.in;
...
public class ChannelledUpdateListener implements Runnable, UpdateListener {
    ...
    private void putMessage(Message.KeyValue value) {
        if (value.getTtl() > 0) {
            ((NotifiableDatastore) datastore).put(value.getKey(), value.getData(), value.getTtl(), TimeUnit.SECONDS, false);
        } else {
            ((NotifiableDatastore) datastore).put(value.getKey(), value.getData(), false);
        }
    }

    private void removeMessage(Message.KeyValue value) {
        ((NotifiableDatastore) datastore).remove(value.getKey(), false);
    }
