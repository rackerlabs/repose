            ...
            <xsl:when test="fn:string(node-name(.)) = 'body'">
                <xsl:copy>
                    <xsl:choose>
                        ...
                        <xsl:when
                                test="fn:ends-with(/httpx/request/head/headers/header[@name='Content-Type']/value, '/json')">
                            <xsl:apply-templates mode="json"/>
                        </xsl:when>
                        <!--<xsl:when test="/httpx/request/head/headers/header[@name='Content-Type']/value = 'application/xml'">-->
                        <xsl:when
                                test="fn:ends-with(/httpx/request/head/headers/header[@name='Content-Type']/value, '/xml')">
