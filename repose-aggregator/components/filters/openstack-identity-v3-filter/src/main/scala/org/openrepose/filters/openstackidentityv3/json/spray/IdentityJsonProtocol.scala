...
object IdentityJsonProtocol extends DefaultJsonProtocol {
  ...
  implicit val projectForAuthenticateResponseFormat = jsonFormat2(ProjectForAuthenticateResponse)
  implicit val roleFormat = jsonFormat(Role, "name", "project_id", "RAX-AUTH:project_id")
  implicit val tokenCredentialsFormat = jsonFormat1(TokenCredentials)
  implicit val userForAuthenticateResponse = jsonFormat(UserForAuthenticateResponse, "id", "name", "RAX-AUTH:defaultRegion")
