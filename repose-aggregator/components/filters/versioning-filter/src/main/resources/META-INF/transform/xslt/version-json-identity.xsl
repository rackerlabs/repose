...
<transform xmlns:ver="http://docs.openstack.org/common/api/v1.0"
    ...
    <template match="@*">
        ...
        <choose>
            <when test="name()='status'">
                <call-template name="do-status-attribute"/>
            </when>
            <otherwise>
                <call-template name="do-attribute"/>
            </otherwise>
        </choose>
    </template>

    <template name="do-status-attribute">
        ...
        <call-template name="json-string">
            <with-param name="in">
                <choose>
                    <when test="$content='DEPRECATED'">deprecated</when>
                    <when test="$content='ALPHA'">alpha</when>
                    <when test="$content='BETA'">beta</when>
                    <when test="$content='CURRENT'">stable</when>
                    <otherwise><value-of select="$content"/></otherwise>
                </choose>
            </with-param>
        </call-template>
    </template>

    <template name="do-attribute">
        <call-template name="json-string">
            <with-param name="in" select="name()"/>
        </call-template>
        <text> : </text>
