...
import org.mockito.Matchers.{any, anyString, argThat, eq => eql, same}
...
class RegexRbacFilterTest
  ...
  before {
    ...
    filter = new RegexRbacFilter(configurationService)
    filter.configurationUpdated(new config.RegexRbacConfig)
  }
  ...
  describe("init method") {
    it("should have the correct default config and schema") {
      ...
      verify(configurationService).subscribeTo(anyString(), eql("regex-rbac.cfg.xml"),
        argThat(hasProperty("path", endsWith("/META-INF/schema/config/regex-rbac.xsd"))),
        any(classOf[UpdateListener[RegexRbacConfig]]), any(classOf[Class[RegexRbacConfig]]))
    }
  }

  describe("doFilter method") {
    it("should pass it on down the filter chain") {
      val chain = mock[FilterChain]

      filter.doFilter(mock[ServletRequest], mock[ServletResponse], chain)

      verify(chain).doFilter(any(classOf[ServletRequest]), any(classOf[ServletResponse]))
    }
  }
