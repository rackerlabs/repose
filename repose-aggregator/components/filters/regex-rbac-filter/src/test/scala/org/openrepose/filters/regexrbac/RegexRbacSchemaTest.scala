...
class RegexRbacSchemaTest extends FunSpec with Matchers {
  val validator = ConfigValidator("/META-INF/schema/config/regex-rbac.xsd")

  describe("schema validation") {
    it("should successfully validate the sample config") {
      validator.validateConfigFile("/META-INF/schema/examples/regex-rbac.cfg.xml")
    }

    it("should successfully validate the config when resources are specified inline") {
      val config = """<regex-rbac xmlns="http://docs.openrepose.org/repose/regex-rbac/v1.0"
