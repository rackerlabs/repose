...
import org.scalatest.{BeforeAndAfter, FunSpec, Matchers => ScalaTestMatchers}
...
class ValkyrieAuthorizationFilterTest extends FunSpec with BeforeAndAfter with MockitoSugar with HttpDelegationManager
  ...
  describe("translates errors states from valkyrie correctly") {
    ...
    List(ValkyrieExpectations(SC_BAD_REQUEST, SC_INTERNAL_SERVER_ERROR),
      ...
      ValkyrieExpectations(429, SC_SERVICE_UNAVAILABLE, Map(HttpHeaders.RETRY_AFTER -> retryTime.toString), Map(HttpHeaders.RETRY_AFTER -> retryTime.toString)),
