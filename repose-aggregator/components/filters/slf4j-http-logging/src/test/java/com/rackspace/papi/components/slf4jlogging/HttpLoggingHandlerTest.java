...
import com.rackspace.papi.components.slf4jlogging.util.FileLogger;
...
public class HttpLoggingHandlerTest {
    public static class WhenHandlingResponse {
        ...
        private List<Logger> buildWrappers(String formatString) {
            final HttpLogFormatter formatter = new HttpLogFormatter(formatString);

            final List<Logger> wrappers = new ArrayList<Logger>();
            Logger loggerWrapper = new Logger(formatter);
        ...
        public void shouldLogTab() throws IOException {
            ...
            new Slf4jHttpLoggingHandler(buildWrappers(readFileAsString(configFile))).handleResponse(request, response);
