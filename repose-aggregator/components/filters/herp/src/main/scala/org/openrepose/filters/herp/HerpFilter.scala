...
class HerpFilter @Inject()(configurationService: ConfigurationService,
  ...
  private def handleResponse(httpServletRequest: HttpServletRequest,
    ...
    val eventValues: Map[String, Any] = Map(
      ...
      "requestGuid" -> stripHeaderParams(httpServletRequest.getHeader(CommonHttpHeader.REQUEST_GUID.toString)),
      "responseGuid" -> stripHeaderParams(httpServletRequest.getHeader(CommonHttpHeader.RESPONSE_GUID.toString)),
