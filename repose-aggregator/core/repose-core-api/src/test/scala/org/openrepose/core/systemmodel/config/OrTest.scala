...
class OrTest extends FunSpec with Matchers with MockitoSugar {
  describe("evaluate") {
    it("should return false when all of the sub criteria are false") {
      ...
      when(filterCriteriaOne.evaluate(any(classOf[HttpServletRequestWrapper]))).thenReturn(false)
      val filterCriteriaTwo = mock[FilterCriterion]
      when(filterCriteriaTwo.evaluate(any(classOf[HttpServletRequestWrapper]))).thenReturn(false)
