  ...
  describe("evaluate") {
    Seq(true, false).foreach { subEvaluate =>
      it(s"should return ${!subEvaluate} when the sub-criteria returns $subEvaluate") {
        val filterCriteria = mock[FilterCriterion]
        when(filterCriteria.evaluate(any(classOf[HttpServletRequestWrapper]))).thenReturn(subEvaluate)
        val filterCriterion = new Not()
        filterCriterion.setFilterCriteria(filterCriteria)
