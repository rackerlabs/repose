...
* *-d* - This flag will run the container in detached mode.
...
* *-v /my/config/directory:/etc/repose* - This option defines a volume that is mounted from the host (i.e., the OS running Docker) onto the Docker container.
...
* *-p 8080:8080* - This option defines a port mapping between the host (i.e., the OS running Docker) and the Docker container.
...
* *rackerlabs/repose:latest* - This is the image ID (in repository:tag format) to create a container from.
