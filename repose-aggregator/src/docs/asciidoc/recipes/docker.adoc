...
<2> This option will run the container in detached mode.
    ...
    If we were to run in the default attached mode instead of optional detached mode, the terminal we used to execute this command would receive all of the container output (e.g. STDOUT, and STDERR).
