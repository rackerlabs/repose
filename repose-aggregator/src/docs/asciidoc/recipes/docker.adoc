...
+++ <details><summary>CentOS</summary><div> +++
----
include::../../../../artifacts/docker/src/docker/centos/Dockerfile[]
----
+++ </div></details> +++
...
. Run the following command from the root of the *Repose* project to build an image based on the Dockerfile copy/pasted from above.
+
...
<4> The last parameter is the directory that contains the Dockerfile.
