...
If delegation is enabled, then the `X-Delegated` header is created for use by the <<herp.adoc#, HERP>> and <<derp.adoc#, DeRP>> filters and is intended for internal processing within Repose.
...
* <<simple-rbac.adoc#, Simple RBAC filter>> - Provides role-based access control to the origin service's API, making use of the `X-PP-Groups` header.
...
| _Request Entity Payload Too Large_ (`413`)

_Too Many Requests_ (`429`)

The Keystone v2 Identity service rate limited the Repose instance.
| _Service Unavailable_ (`503`)
| _Service Unavailable_ (`503`)
| _Service Unavailable_ (`503`)

| _Internal Server Error_ (`500`)

_Not Implemented_ (`501`)
...
----
<1> Keystone v2 Identity service Endpoint URI.

=== Using an admin account (not recommended)
This configuration will uses an admin account instead of using the self-validating tokens feature.
...
----
<1> Admin username to access Keystone v2 Identity service.
<2> Admin password to access Keystone v2 Identity service.
<3> Keystone v2 Identity service Endpoint URI.
...
----
<1> Keystone v2 Identity service Endpoint URI.
<2> Http Connection pool ID to use when talking to Keystone v2 Identity service.
...
<6> Indicates whether or not to include the `apply_rcn_roles` query parameter when talking to Keystone v2 Identity service. (Default: false)

=== Enable Delegation
In some cases, you may want to delegate the decision to authenticate a request down the chain to either another filter or to the origin service.
This filter allows an unauthenticated request to pass as either confirmed or indeterminate when placed in delegating mode.
The filter sends a confirmed value when valid credentials have been confirmed by the Keystone v2 Identity service, and sends an indeterminate value when the Keystone v2 Identity service can not confirm the credentials.
...
If this filter is configured to run in delegating mode, the `X-Identity-Status` header is set to a value of indeterminate.
The request is then passed along to subsequent filters and the origin service with the `X-Delegated` header set.
...
When setting up a chain of delegating filters the highest quality number will be the one that is eventually output. (Default: 0.7)
...
The whitelist contains a list of regular expressions that Repose attempts to match against the full request URI.
If the URI matches a regular expression on the white list, the request is passed to the origin service.
...
----
<1> The regular expression to allow matching URI's to pass without requiring authentication.

=== Configuring Cache Timeouts
This filter caches authentication tokens.
The length of time that tokens are cached is determined by the Time To Live (TTL) value that is returned from the authentication service (e.g., a Keystone v2 Identity service) during token validation.
...
----
<1> This value will be added or subtracted to the cache timeouts to make sure that the cached items have some variability so they don't all expire at the exact same time. (Default: 0)
...
* If `0`, data is cached indefinitely.
...
<5> The Java Regular Expression with a single matching group to extract the Tenant ID from the URI for validation.
<6> If this element is included, then include Quality parameters on all the tenant ID headers sent.
<7> The default tenant has the highest quality. (Default: 0.9)
<8> Followed by the one that matches the URI (if any). (Default: 0.7)
<9> Followed by the tenants from the roles. (Default: 0.5)

[NOTE]
====
The `${your-regex}` should be replaced by a Java Regular Expression that contains a capture group around the portion of the URI where the tenant ID is expected.
This will populate the `X-Tenant-Id` header with the value in the capture group.
