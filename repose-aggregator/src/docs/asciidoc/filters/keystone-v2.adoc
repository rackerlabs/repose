...
----
<1> Indicates if all the Tenant IDs from the user and the roles the user has should be sent or not. +
    Default: `false`
...
<4> If in legacy roles mode, then all roles associated with a user token are forwarded.
    ...
    Default: `false`
<5> The https://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html[Java Regular Expression] with at least one capture group.
...
<7> The default tenant has the highest quality by default. +
    Default: `0.9`
<8> Followed by the one that matches the URI by default (if any). +
    Default: `0.7`
<9> Followed by the tenants from the roles by default. +
    Default: `0.5`
