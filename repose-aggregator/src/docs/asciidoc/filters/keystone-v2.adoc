...
<2> Indicates if all the Tenant IDs from the user and the roles the user has should be sent or not. +
    Default: `false`
<3> If this element is included, then Tenant ID Validation will be enforced based on the value extracted from the URI.
...
<8> The default tenant has the highest quality by default. +
    Default: `0.9`
<9> Followed by the one that matches the URI by default (if any). +
    Default: `0.7`
