...
* `X-Map-Roles` - Identifies the tenant-to-role mapping for all of the user's tenants and roles.
  This header will be a base 64 encoded JSON map of strings to arrays of strings (e.g., the base 64 encoding of `{"someTenant": ["someRole", "sharedRole"], "otherTenant": ["otherRole", "sharedRole"]}`).
