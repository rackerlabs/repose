...
<6> The https://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html[Java Regular Expression] with at least one capture group.
    The text from each capture group will be treated as a Tenant ID for validation.
...
[NOTE]
====
If the default tenant and the tenant(s) extracted from the request are the same, then the highest quality between the two will be used.
====
...
The `uri-extraction-regex` will be used to populate the `X-Tenant-ID` header with the value(s) extracted by all capturing groups.
====
...
A `uri-extraction-regex` may also have multiple capture groups.
All captured values will be validated.
====
