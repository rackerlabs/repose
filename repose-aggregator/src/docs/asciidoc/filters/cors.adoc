...
* *Schema:* link:../schemas/cors-configuration.xsd[cors-configuration.xsd]
...
[CAUTION]
====
If a proxy server sitting between the client and Repose rewrites the `Host` without updating the `X-Forwarded-Host` header with the original value,
Repose will not be able to correctly identify same-origin requests coming from that client.
