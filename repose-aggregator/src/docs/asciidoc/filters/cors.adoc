...
* link:../schemas/cors-configuration.xsd[Schema]
...
This should be one of the first filters, if not the first filter, in the filter chain in order to properly handle Preflight CORS requests.
...
[NOTE]
====
Preflight Requests are completely handled by the CORS filter.
No other filters after the CORS filter will process them, and they won't reach the origin service.
====
...
[NOTE]
====
Some web browsers (e.g. Chrome and Safari) will send the `Origin` header for same-origin requests.
...
[NOTE]
====
If your origin service requires a special header (e.g. `X-Auth-Token`),
