...
For information about using the Flume with the HERP filter for user access event recording, see link:../recipes/api-event-flume.html[CF Flume Sink].
...
| projectID            | The tenant ID or project ID of the user making the request authenticated through an OpenStack Identity service.
                         Note that, if the client-auth filter is used, the tenantId from that filter will be inserted into this field.                      | client-auth *or* openstack-identity-v3
...
Repose has developed link:../recipes/api-event-flume.html[CF Flume Sink] to eliminate disk space and log file issues.
