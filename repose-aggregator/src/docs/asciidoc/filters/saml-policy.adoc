...
The request body will be decoded and replaced with an xml version.
...
a| * If a policy that matches the assertion issuer can not be found.
   ...
   * If Identity returns does not return an IDP ID when requested.

| `405`
| If the request HTTP method is not `POST`.

| `415`
| If the request `Content-Type` header does not have a value of `application/x-www-form-urlencoded`.
...
| `502`
a| * If Identity returns a 5xx response.
   * If parsing the policy fails.

| `502`
| If Identity returns a 413 or 429 response code, indicating that this filter's request was rate limited.
