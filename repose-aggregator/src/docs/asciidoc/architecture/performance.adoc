...
image::perf-test-design-diagram.png[Performance Test Framework Design Diagram]
...
Most if not all of the configuration files that were referred to above will be Jinja2 files, distinguishable by the `.j2` file extension.
====
...
[NOTE]
====
We use of Puppet to manage our performance test environment.
...
After test execution has completed, Gatling will generate a HTML report.
