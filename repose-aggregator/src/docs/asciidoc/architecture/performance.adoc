...
contains the `category` sub-directories `filters/` and `use-cases/`.
...
[NOTE]
====
The `dir_prefix` is the `{category}/{test}` used to create the new vars file (e.g. `filter/saml`).
====
...
[NOTE]
====
The `package` is similarly created using `{category}.{test}` (e.g. `filter.saml`).
====
