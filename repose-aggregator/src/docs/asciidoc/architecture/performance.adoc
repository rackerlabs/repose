...
.. At a minimum, it should contain the `dir_prefix`, a filter chain, and the Gatling simulation name and package.
...
Template files should be placed under the `templates/config/` directory while raw configuration files should be placed under the `files/config/` directory.
...
ansible-playbook start_test.yml \ # <1>
  -e '{perf_test: filters/saml, gatling: {test: {params: {duration_in_min: 15, saml: {num_of_issuers: 2}}}}}' \ # <2> <3>
  -vvvv # <4>
```
...
ansible-playbook start_test.yml \
  -e '{perf_test: use-cases/simple, gatling: {test: {params: {duration_in_min: 15, warmup_duration_in_min: 5}}}}' \
  -vvvv
```
...
```
ansible-playbook start_test.yml \
  -e '{perf_test: filters/saml, cloud: {server: {repose: {flavor: performance1-4}}}}' \
  -vvvv
```
...
```
ansible-playbook start_test.yml \
  -e '{perf_test: filters/saml, repose: {service: {start_timeout_in_sec: 900}}}' \
  -vvvv
```
...
```
ansible-playbook start_test.yml \
  -e '{perf_test: filters/saml, repose: {systemd_opts: {use_saxon: true}}}' \
  -vvvv
```
...
```
ansible-playbook start_test.yml \
  -e '{perf_test: filters/saml, repose: {systemd_opts: {java_opts: "-Xmx2g -Xms2g -XX:+UseG1GC -XX:+UseStringDeduplication"}}}' \
  -vvvv
```
...
```
ansible-playbook start_test.yml \
  -e '{perf_test: filters/saml, repose: {service: {state: restarted}, systemd_opts: {use_saxon: true}}}' \
  -vvvv
```
...
```
ansible-playbook start_test.yml \
  -e '{perf_test: filters/saml, gatling: {results: {output_dir: custom-base-name}}}' \
  -vvvv
```
...
```
ansible-playbook start_test.yml \
  -e '{perf_test: filters/saml, cloud: {naming_prefix: perf-saml-many-issuers}, gatling: {test: {params: {saml: {num_of_issuers: 100}}}}}' \
  -vvvv
```
...
ansible-playbook stop_test.yml \
  -e '{perf_test: filters/saml, cloud: {naming_prefix: perf-saml-many-issuers}}' \
  -vvvv
```
...
```bash
ansible-playbook start_test.yml \
  -e '{perf_test: filters/scripting, script_lang: python}' \
  -vvvv
```
