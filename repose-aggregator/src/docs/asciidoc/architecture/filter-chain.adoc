...
It's based on and uses the same contract as the https://docs.oracle.com/javaee/6/api/javax/servlet/FilterChain.html[JEE Filter Chain].
...
You can decide whether or not a filter should run based on it's URI using a https://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html[regular expression].
...
...
----
<1> This filter will only be run when the request is to the `/foo` resource.
