...
<2> IF this attribute is configured,
    ...
    NOTE: IF the `keystore-filename` is defined,
...
<11> Overrides the default `port`, defined for this `cluster` 's datasotores to listen, only for this particular `node`.
...
Note that distributed datastore will use a connection pool to communicate across nodes.
...
At current the only supported hashing algorithm is MD5.
...
=== Remote Management
The repose distributed datastore component is a filter component that hosts a simple RESTful API that can be contacted to perform remote object store operations.
...
In order to keep healthy nodes from attempting to route requests to the damaged node, a participating node may tell it's destination that the destination may not route the request and must store, delete or get the value locally.
...
=== Rate Limiting
Rate limiting can be configured to use any of the three to store rate limiting information for users.
See the documentation for Rate Limiting to find out how to specify which datastore to use.
By default, rate limiting will use the distributed datastore if available.
...
The Remote Datastore Service communicates to the configure host through the configured port.
...
NOTE: Refer to the <<http-connection-pool.adoc#,http-connection-pool.cfg.xml>> documentation for setting up Client Authentication on the connection pool used to connect to the Remote Datastore.
